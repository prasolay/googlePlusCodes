...
df1 = pd.read_csv("data\Tainan_103.csv", encoding="big5")
# df2 = pd.DataFrame([df1.iloc['WKT'].str.split()], columns=["Latitude", "Longitude"])

# print(df1['WKT'])
print("--------------------")
df1[['lon', 'lat']] = df1['center'].str.split(' ', expand=True)
...
df1["pluscode"] = df1.apply(lambda row: olc.encode(row['lat'], row['lon']), 'columns')
# df1["googlePlus"] =  olc.encode(df1['lat'], df1['lon'])
print(df1["pluscode"][1])
print("--------------")
...
def createEid(inputDF, countyName, year):
    ...
    for index,  pluscode in enumerate(inputDF["pluscode"]):
        inputDF.at[index, "eid"] = countyCodeName + "-" + year + "-" + str(index+1).zfill(5)
    
    return inputDF

    
df2 = createEid(df1, '臺南市', '103')

print(df2.loc[0:1])
print("-------------")

# #output to csv
df2.to_csv("data\\output\\GPC_Tainan103.csv", encoding="big5")
